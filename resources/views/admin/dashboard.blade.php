    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-white/60 mt-2">Overview of your appointment system</p>
            </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="bg-yellow-500/10 rounded-lg p-4 border border-yellow-500/20">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-yellow-400">{{ __('Pending') }}</p>
                                    <p class="text-2xl font-bold text-yellow-300">{{ number_format($appointmentStats['pending'] ?? 0) }}</p>
                                </div>
                        ...
                        <div class="bg-blue-500/10 rounded-lg p-4 border border-blue-500/20">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-blue-400">{{ __('Confirmed') }}</p>
                                    <p class="text-2xl font-bold text-blue-300">{{ number_format($appointmentStats['confirmed'] ?? 0) }}</p>
                                </div>
                        ...
                        <div class="bg-green-500/10 rounded-lg p-4 border border-green-500/20">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-green-400">{{ __('Completed') }}</p>
                                    <p class="text-2xl font-bold text-green-300">{{ number_format($appointmentStats['completed'] ?? 0) }}</p>
                                </div>
                        ...
                        <div class="bg-red-500/10 rounded-lg p-4 border border-red-500/20">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-red-400">{{ __('Cancelled') }}</p>
                                    <p class="text-2xl font-bold text-red-300">{{ number_format($appointmentStats['cancelled'] ?? 0) }}</p>
                                </div>
                ...
                <div class="glass-card rounded-xl p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="bg-green-500/10 rounded-lg p-4 border border-green-500/20">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-green-400">{{ __('Paid') }}</p>
                                    <p class="text-2xl font-bold text-green-300">{{ number_format($paymentStats['paid'] ?? 0) }}</p>
                                </div>
                                <div class="bg-green-500/20 rounded-full p-3">
                                    <svg class="h-6 w-6 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        ...
                        <div class="bg-red-500/10 rounded-lg p-4 border border-red-500/20">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-red-400">{{ __('Unpaid') }}</p>
                                    <p class="text-2xl font-bold text-red-300">{{ number_format($paymentStats['unpaid'] ?? 0) }}</p>
                                </div>
                                <div class="bg-red-500/20 rounded-full p-3">
                                    <svg class="h-6 w-6 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <!-- Refunded -->
                        <div class="bg-white/5 rounded-lg p-4 border border-white/10 col-span-2">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-white/60">{{ __('Refunded') }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                <a href="{{ route('users.index') }}" class="flex items-center justify-center bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200">
                    <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                    </svg>
                    {{ __('Manage Users') }}
                </a>
                <a href="{{ route('providers.index') }}" class="flex items-center justify-center bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200">
                    <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                    </svg>
                    {{ __('Manage Providers') }}
                </a>
                <a href="{{ route('appointments.index') }}" class="flex items-center justify-center bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200">
                    <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                    </svg>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-white/10">
                            <thead class="bg-white/5">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/60 uppercase tracking-wider">{{ __('Client') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/60 uppercase tracking-wider">{{ __('Provider') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/60 uppercase tracking-wider">{{ __('Service') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/60 uppercase tracking-wider">{{ __('Date & Time') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/60 uppercase tracking-wider">{{ __('Status') }}</th>
                                </tr>
                            </thead>
