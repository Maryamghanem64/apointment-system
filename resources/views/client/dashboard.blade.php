...
<div class="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        @php
            $stats = [
                ['label' => 'Upcoming', 'value' => $upcomingCount ?? 0, 'color' => 'blue', 'icon' => '📅'],
                ['label' => 'Pending', 'value' => $pendingCount ?? 0, 'color' => 'amber', 'icon' => '⏳'],
                ['label' => 'Completed', 'value' => $completedCount ?? 0, 'color' => 'emerald', 'icon' => '✓'],
                ['label' => 'Cancelled', 'value' => $cancelledCount ?? 0, 'color' => 'rose', 'icon' => '✕'],
            ];
        @endphp
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-white/40 text-xs mt-1 flex items-center gap-1">
                        🕐 {{ \Carbon\Carbon::parse($nextAppointment->appointment_date)->format('M d, Y — h:i A') }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 flex-wrap">
                ...
                @if($nextAppointment->status === 'confirmed' && !$nextAppointment->payment)
                    <a href="{{ route('payments.show', $nextAppointment) }}"
                       class="bg-gradient-to-r from-blue-500 to-cyan-400 text-white text-xs font-medium rounded-xl px-4 py-2 hover:shadow-lg hover:shadow-cyan-500/25 transition-all duration-300">
                        💳 Pay Now
                    </a>
                ...
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-1 mt-0.5">
                            ...
                            @for($i = 1; $i <= 5; $i++)
                                <span class="text-sm {{ $i <= $avg ? 'text-cyan-400' : 'text-white/20' }}">★</span>
                            @endfor
    ...
    <div class="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 mb-6">
        ...
        <div class="flex items-start gap-4 py-4 border-b border-white/5 last:border-0">
            <div class="flex gap-0.5 shrink-0">
                @for($i = 1; $i <= 5; $i++)
                    <span class="text-lg {{ $i <= $review->rating ? 'text-cyan-400' : 'text-white/20' }}">★</span>
                @endfor
            </div>
            <div class="flex-1">
